...
const Dialog: React.FunctionComponent<{ onClose: () => void }> = props => {
  ...
  return ReactDOM.createPortal(
    <div className='r-parts-citySelect-dialog'>
      <header>
      ...
      <CurrentLocation />
      <h2>全部城市</h2>
      <ol className='r-parts-citySelect-index'>
        {indexList.map(a => (
          <li key={a}>{a}</li>
        ))}
      </ol>
      <div className='cityList'>
        {cityList.map(([letter, list]) => {
          return (
            <div key={letter} className='r-parts-citySelect-citySection'>
              <h4>{letter}</h4>
              {list.map(city => {
                return (
                  <div
                    onClick={() => onClick(city)}
                    className='r-parts-citySelect-cityName'
                    key={city}
                  >
                    {city}
                  </div>
...
const CurrentLocation: React.FunctionComponent = () => {
  ...
  return <div className='currentCity'>当前城市{city}</div>;
};
export { CitySelect };
